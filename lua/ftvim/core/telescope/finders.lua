...
function M.find_ftvim_files(opts)
  opts = opts or {}
  local theme_opts = themes.get_ivy {
    sorting_strategy = "ascending",
    layout_strategy = "bottom_pane",
    prompt_prefix = ">> ",
    prompt_title = "~ FtVim files ~",
    cwd = get_runtime_dir(),
    search_dirs = { get_ftvim_base_dir(), ftvim.lsp.templates_dir },
  }
  opts = vim.tbl_deep_extend("force", theme_opts, opts)
  builtin.find_files(opts)
end

function M.grep_ftvim_files(opts)
  opts = opts or {}
  local theme_opts = themes.get_ivy {
    sorting_strategy = "ascending",
    layout_strategy = "bottom_pane",
    prompt_prefix = ">> ",
    prompt_title = "~ search FtVim ~",
    cwd = get_runtime_dir(),
    search_dirs = { get_ftvim_base_dir(), ftvim.lsp.templates_dir },
  }
  opts = vim.tbl_deep_extend("force", theme_opts, opts)
...
function M.view_ftvim_changelog()
  local opts = themes.get_ivy {
    cwd = get_ftvim_base_dir(),
  }
  opts.entry_maker = make_entry.gen_from_git_commits(opts)

  pickers
    .new(opts, {
      prompt_title = "~ FtVim Changelog ~",
