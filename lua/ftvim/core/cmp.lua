...
M.config = function()
	...
	local cmp = require("ftvim.utils.modules").require_on_index("cmp")
	local luasnip = require("ftvim.utils.modules").require_on_index("luasnip")
	...
	ftvim.builtin.cmp = {
		...
		enabled = function()
			...
			return ftvim.builtin.cmp.active
		end,
		confirm_opts = {
		...
		formatting = {
			...
			format = function(entry, vim_item)
				local max_width = ftvim.builtin.cmp.formatting.max_width
				if max_width ~= 0 and #vim_item.abbr > max_width then
					vim_item.abbr = string.sub(vim_item.abbr, 1, max_width - 1) .. ftvim.icons.ui.Ellipsis
				end
				if ftvim.use_icons then
					vim_item.kind = ftvim.builtin.cmp.formatting.kind_icons[vim_item.kind]

					if entry.source.name == "copilot" then
						vim_item.kind = ftvim.icons.git.Octoface
						vim_item.kind_hl_group = "CmpItemKindCopilot"
					end

					if entry.source.name == "cmp_tabnine" then
						vim_item.kind = ftvim.icons.misc.Robot
						vim_item.kind_hl_group = "CmpItemKindTabnine"
					end

					if entry.source.name == "crates" then
						vim_item.kind = ftvim.icons.misc.Package
						vim_item.kind_hl_group = "CmpItemKindCrate"
					end

					if entry.source.name == "lab.quick_data" then
						vim_item.kind = ftvim.icons.misc.CircuitBoard
						vim_item.kind_hl_group = "CmpItemKindConstant"
					end

					if entry.source.name == "emoji" then
						vim_item.kind = ftvim.icons.misc.Smiley
						vim_item.kind_hl_group = "CmpItemKindEmoji"
					end
				end
				vim_item.menu = ftvim.builtin.cmp.formatting.source_names[entry.source.name]
				vim_item.dup = ftvim.builtin.cmp.formatting.duplicates[entry.source.name]
					or ftvim.builtin.cmp.formatting.duplicates_default
				return vim_item
			end,
		},
		snippet = {
		...
		mapping = cmp_mapping.preset.insert({
			...
			["<CR>"] = cmp_mapping(function(fallback)
				if cmp.visible() then
					local confirm_opts = vim.deepcopy(ftvim.builtin.cmp.confirm_opts) -- avoid mutating the original opts below
