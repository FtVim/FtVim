...
function M.detectors.lsp(buf)
  ...
  for _, client in pairs(FtVim.lsp.get_clients({ bufnr = buf })) do
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  FtVim.info(lines, { title = "FtVim Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("LazyRoot", function()
    FtVim.root.info()
  end, { desc = "FtVim roots for the current buffer" })
  ...
  vim.api.nvim_create_autocmd({ "LspAttach", "BufWritePost", "DirChanged", "BufEnter" }, {
    group = vim.api.nvim_create_augroup("ftvim_root_cache", { clear = true }),
...
function M.get(opts)
  ...
  return FtVim.is_win() and ret:gsub("/", "\\") or ret
end
