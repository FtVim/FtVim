...
function M.rename_file()
  ...
  local old = assert(FtVim.root.realpath(vim.api.nvim_buf_get_name(buf)))
  local root = assert(FtVim.root.realpath(FtVim.root.get({ normalize = true })))
...
function M.formatter(opts)
  ...
  local ret = {
    ...
    priority = 1,
    format = function(buf)
      M.format(FtVim.merge({}, filter, { bufnr = buf }))
    end,
    sources = function(buf)
      local clients = M.get_clients(FtVim.merge({}, filter, { bufnr = buf }))
      ---@param client vim.lsp.Client
  ...
  return FtVim.merge(ret, opts) --[[@as LazyFormatter]]
end
...
function M.format(opts)
  opts = vim.tbl_deep_extend(
    ...
    {},
    opts or {},
    FtVim.opts("nvim-lspconfig").format or {},
    FtVim.opts("conform.nvim").format or {}
  )
  ...
  M.on_supports_method("textDocument/documentHighlight", function(_, buf)
    vim.api.nvim_create_autocmd({ "CursorHold", "CursorHoldI", "CursorMoved", "CursorMovedI" }, {
      ...
      callback = function(ev)
        if not require("ftvim.plugins.lsp.keymaps").has(buf, "documentHighlight") then
          return false
        end
        ...
        if not ({ M.words.get() })[2] then
          if ev.event:find("CursorMoved") then
            vim.lsp.buf.clear_references()
          elseif not FtVim.cmp.visible() then
