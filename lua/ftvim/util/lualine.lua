...
function M.cmp_source(name, icon)
  ...
  local colors = {
    ok = FtVim.ui.fg("Special"),
    error = FtVim.ui.fg("DiagnosticError"),
    pending = FtVim.ui.fg("DiagnosticWarn"),
  }

  return {
    function()
      return icon or FtVim.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
    end,
    cond = function()
      return status() ~= nil
    end,
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    local root = FtVim.root.get({ normalize = true })
    local cwd = FtVim.root.cwd()
