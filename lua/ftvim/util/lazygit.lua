...
M.theme_path = FtVim.norm(vim.fn.stdpath("cache") .. "/lazygit-theme.yml")
...
function M.open(opts)
  if vim.g.lazygit_theme ~= nil then
    FtVim.deprecate("vim.g.lazygit_theme", "vim.g.lazygit_config")
  end
  ...
  if vim.g.lazygit_config then
    ...
    if not M.config_dir then
      ...
      if ok then
        ...
        vim.env.LG_CONFIG_FILE = FtVim.norm(M.config_dir .. "/config.yml" .. "," .. M.theme_path)
        local custom_config = FtVim.norm(M.config_dir .. "/custom.yml")
  ...
  return FtVim.terminal(cmd, opts)
end
...
function M.get_color(v)
  ...
  local color = {}
  if v.fg then
    color[1] = FtVim.ui.color(v.fg)
  elseif v.bg then
    color[1] = FtVim.ui.color(v.bg, true)
  end
  if v.bold then
...
function M.blame_line(opts)
  ...
  local root = FtVim.root.detectors.pattern(0, { ".git" })[1] or "."
...
function M.browse()
  ...
  local function open(remote)
    if remote then
      FtVim.info(("Opening [%s](%s)"):format(remote.name, remote.url))
  ...
  if #remotes == 0 then
    return FtVim.error("No git remotes found")
