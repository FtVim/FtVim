...
---@class ftvim.Toggle.wrap: ftvim.Toggle
---@operator call:boolean

---@param toggle ftvim.Toggle
function M.wrap(toggle)
  return setmetatable(toggle, {
    __call = function()
      ...
      if state then
        FtVim.info("Enabled " .. toggle.name, { title = toggle.name })
      else
        FtVim.warn("Disabled " .. toggle.name, { title = toggle.name })
      end
      return state
    end,
  }) --[[@as ftvim.Toggle.wrap]]
end
...
function M.map(lhs, toggle)
  ...
  FtVim.safe_keymap_set("n", lhs, function()
    t()
...
function M.wk(lhs, toggle)
  if not FtVim.has("which-key.nvim") then
    return
  end
  local function safe_get()
    local ok, enabled = pcall(toggle.get)
    if not ok then
      FtVim.error({ "Failed to get toggle state for **" .. toggle.name .. "**:\n", enabled }, { once = true })
    end
    return enabled
  end
...
function M.format(buf)
  return M.wrap({
    ...
    get = function()
      ...
      return FtVim.format.enabled()
    end,
    set = function(state)
      FtVim.format.enable(state, buf)
    end,
  })
end
  ...
  set = function(state)
    if state then
      ...
      vim.api.nvim_create_autocmd("ExitPre", {
        once = true,
        group = vim.api.nvim_create_augroup("ftvim_restore_max_exit_pre", { clear = true }),
