...
function M.formatexpr()
  if FtVim.has("conform.nvim") then
...
function M.info(buf)
  ...
  FtVim[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    FtVim.warn("No formatter available", { title = "FtVim" })
  end
end
...
function M.health()
  ...
  local has_extra = vim.tbl_contains(Config.spec.modules, "ftvim.plugins.extras.lsp.none-ls")
  if has_plugin and not has_extra then
    FtVim.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in FtVim.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to enable the `ftvim.plugins.extras.lsp.none-ls` extra,",
