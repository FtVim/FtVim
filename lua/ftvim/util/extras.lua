...
---@class ftvim.util.extras
local M = {}
M.buf = 0

---@type LazyExtraSource[]
M.sources = {
  { name = "FtVim", desc = "FtVim extras", module = "ftvim.plugins.extras" },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("ftvim.extras")
---@type string[]
M.state = nil
...
function M.wants(opts)
  ...
  if opts.root then
    ...
    return #FtVim.root.detectors.pattern(M.buf, opts.root) > 0
  end
  return false
end
...
function M.get()
  ...
  for _, source in ipairs(M.sources) do
    local root = FtVim.find_root(source.module)
    if root then
      FtVim.walk(root, function(path, name, type)
...
function M.get_extra(source, modname)
  ...
  return {
    ...
    managed = vim.tbl_contains(FtVim.config.json.data.extras, modname) or not enabled,
    plugins = plugins,
    optional = optional,
  }
end
...
function X.new()
  ...
  self.float = Float.new({ title = "FtVim Extras" })
...
function X:toggle()
  ...
  for _, extra in ipairs(self.extras) do
    if extra.row == pos[1] then
      ...
      FtVim.config.json.data.extras = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, FtVim.config.json.data.extras)
      M.state = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, M.state)
      if extra.enabled then
        table.insert(FtVim.config.json.data.extras, extra.module)
        M.state[#M.state + 1] = extra.module
      end
      table.sort(FtVim.config.json.data.extras)
      FtVim.json.save()
      FtVim.info(
        "`"
          .. extra.name
          ...
          .. "\nPlease restart FtVim to apply the changes.",
        { title = "LazyExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("FtVim Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled FtVim extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  if not extra.managed then
    ...
    elseif vim.tbl_contains(FtVim.plugin.core_imports, extra.module) then
      self:diagnostic({
  ...
  if extra.source.name ~= "FtVim" then
  ...
  for _, import in ipairs(extra.imports) do
    import = import:gsub("^ftvim.plugins.extras.", "")
