...
local function git_cmd(opts)
  ...
  opts = opts or {}
  opts.cwd = opts.cwd or get_ftvim_base_dir()
...
---@param branch string
function M.switch_ftvim_branch(branch)
  if not safe_deep_fetch() then
    return
  end
...
---@return string|nil
function M.get_ftvim_description()
  local _, results = git_cmd { args = { "describe", "--dirty", "--always" } }

  local description = if_nil(results[1], M.get_ftvim_branch())
  return description
end

---Get currently running version of Lunarvim
---@return string
function M.get_ftvim_version()
  local current_branch = M.get_ftvim_branch()

  local ftvim_version
  if current_branch ~= "HEAD" or "" then
    ftvim_version = current_branch .. "-" .. M.get_ftvim_description()
  else
    ftvim_version = "v" .. M.get_ftvim_tag()
  end
  return ftvim_version
end
